...
const NuevoPlan = ({ userId }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (userSnap.exists()) {
                // 4. Si el documento existe, actualizarlo
                await updateDoc(userRef, {
                    assignedPlans: planRef.id,
                    estadoPlan: 'Activo',
                    fecha: fecha
                })
            } else {
            ...
            // const planActivoRef = doc(db, 'planesActivos', userId)
            ...
            alert('Plan creado y asignado exitosamente')
            navigate('/dashboard')
        } catch (error) {
    ...
    return (
        <>
            <div className='container flex flex-col items-center justify-center mx-auto gap-5 rounded-md bg-zinc-800 p-5'>
                <h1 className='text-2xl font-bold text-center text-white'>Nuevo plan</h1>
